...
export class PayPalService {
  ...
  constructor(private configService: ConfigService) {
    this.PAYPAL_API_BASE_URL = this.configService.get<string>(
      'PAYPAL_API_BASE_URL',
  ...
  public async generateAccessToken(): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new InternalServerErrorException(
        `Failed to generate PayPal access token: ${JSON.stringify(errorData)}`,
      );
    }
  ...
  async createOrder(amount: number, currency: string = 'USD'): Promise<any> {
    ...
    const response = await fetch(
      `${this.PAYPAL_API_BASE_URL}/v2/checkout/orders`,
      {
        method: 'POST',
        headers: {
  ...
  async captureOrder(orderId: string): Promise<any> {
    const accessToken = await this.generateAccessToken();
    const response = await fetch(
      `${this.PAYPAL_API_BASE_URL}/v2/checkout/orders/${orderId}/capture`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  async verifyWebhookSignature(headers: any, body: any): Promise<boolean> {
    ...
    const response = await fetch(
      `${this.PAYPAL_API_BASE_URL}/v1/notifications/verify-webhook-signature`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
